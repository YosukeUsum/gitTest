...
### 仕様追加・実装: 2人対戦モード(お邪魔行攻撃)の新設(FR-17〜FR-22, NFR-14〜16, AC-14〜19)

- 要望: 「二人で対戦できるように仕様を変更して、対戦仕様は例えば1行消したら相手にも
  一行おじゃまとして1列降らせるような仕様で」。対戦方式・お邪魔行の反映タイミング・
  送信数ルール・相殺ルールの4点をユーザーに確認したうえで仕様化した。
  壁打ちログ: `docs/spec-log/2026-09-12-two-player-battle-mode.md`。
- 対応(仕様):
  1. `SPEC.md` に FR-17〜FR-22(モード起動・独立した盤面/スコア・非線形の攻撃力
     テーブル(1/2/3/4行消去→0/1/2/4行)・保留お邪魔行の相殺と反映タイミング・
     お邪魔行の形状・勝敗判定)、NFR-14〜NFR-16(ロジックのcurses非依存・UI層への
     隔離・キー重複禁止)、AC-14〜AC-19を追加。
  2. 2章(目的・スコープ)・3章(用語定義: お邪魔行/相殺/保留お邪魔行数)・6章
     (アーキテクチャ、`battle.py`/`test_battle.py`を追加)・7章(データモデル、
     `BattleState`を追加)・8章(第2プレイヤーのキーバインド: A/D/S/W/E/F)・12章
     (改訂履歴)を更新。
- 対応(実装):
  1. `src/tetris_cli/board.py`: `add_garbage_rows()` を追加(盤面上部からの
     切り捨て+最下部への追加、1行につき1列だけ空ける)。お邪魔行の色ID
     `GARBAGE_COLOR` を新設。
  2. `src/tetris_cli/game.py`: `Game` に `on_locked(cleared)` / `on_before_spawn()`
     フックを追加(単体では未使用、curses・お邪魔行のいずれにも非依存)。ロック時に
     消去行数を通知し、次のミノ出現直前(ライン消去エフェクト完了後を含む)に
     お邪魔行の反映機会を設ける構造にした。
  3. `src/tetris_cli/battle.py`(新規): `BattleGame` が2つの `Game` を保持し、上記
     フック経由で攻撃力算出・相殺・保留キュー・盤面反映・勝敗判定(`winner`/
     `is_over`)を行う。ライン消去エフェクトと競合しないよう、お邪魔行の盤面反映は
     必ず次のミノ出現の直前(エフェクト完了後)に行うよう設計した(詳細は壁打ちログの
     「根拠」を参照)。
  4. `src/tetris_cli/ui_curses.py`: `run_battle()` を追加し、2枚の盤面を横に並べて
     描画、第2プレイヤー用キー入力(`P2_KEYMAP`)を振り分ける。お邪魔行は既存の白
     (Lミノ)を流用しつつ `curses.A_DIM` で区別する専用カラーペアを追加した。
     `main()` はコマンドライン引数 `--2p` の有無で `run`/`run_battle` を切り替える
     (FR-17)。
  5. `tests/test_battle.py`(新規、11件): AC-14〜AC-19に対応するテストを追加
     (攻撃力テーブル、相殺の一部/超過、保留の盤面反映とリセット、お邪魔行の形状、
     勝敗判定、および `Game.hard_drop()` からの実際のロックを通した結合テスト)。
- テスト: この時点で `pytest` が利用できない開発環境だったため、`pytest` と同等の
  assertベースの簡易テストランナーで代替実行し、既存56件+新規11件の計67件すべてが
  成功することを確認した(正式な `pytest` によるログ記録(NFR-6)は未実施であり、
  本CHANGELOGはその事実を正確に記載する)。
- 評価者レビュー(NFR-13)について: `tests/test_battle.py` を新規作成したため本来は
  `pytest` 合格確認後に評価者レビューを別セッションで実行すべきだが、このセッションから
  到達可能な環境では `claude` コマンドが無効化されており、別セッションでのレビューアー
  起動ができなかった。ユーザー側の端末(`claude --agent reviewer` 等)で改めて評価者
  レビューを実施することを推奨する。
- `docs/design/` 配下の説明資料(概要資料・設計書・要件一覧)は、本コミットに続けて
  NFR-12に従い更新する。
